...
public class ArrayDeque<Item> {
	...
	private void reSize(double coefficient){
		Item[] stdOrderItems = stdOrder();
		this.items = (Item[])new Object[(int) (items.length*coefficient)];
	...
	/*Romove the item from the very beginning of the list*/
	public Item removeFirst(){
		size -= 1;
		nextFirst = aListNumber(nextFirst+1);
		Item originFirst = items[nextFirst];
	...
	/*Romove the item from the very end of the list*/
	public Item removeLast(){
		size -= 1;
		nextLast -= 1;
		Item originLast = items[nextLast];
		items[nextLast] = null;
		if(isBelowUsage()){
			reSize(0.5);
		}
