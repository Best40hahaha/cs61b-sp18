...
public class Room {
  ...
  public static Double distance(Room room1, Room room2) {
    List<Position> b1 = room1.border();
    List<Position> b2 = room2.border();
    Double minDistance = null;
    for (Position p1 : b1) {
      for (Position p2 : b2) {
        double d = Math.pow((p1.widthPosition - p2.widthPosition), 2) + Math
            .pow((p1.heightPosition - p2.heightPosition), 2);
        if (minDistance == null) {
          minDistance = d;
        }
  ...
  public static Integer[] nearRoomIndex(List<Room> rooms1, List<Room> rooms2) {
    Double minDistance = null;
    Integer[] index = new Integer[2];
    for (int i = 0; i < rooms1.size(); i++) {
      for (int j = 0; j < rooms2.size(); j++) {
        Double d = Room.distance(rooms1.get(i), rooms2.get(j));
        if (minDistance == null) {
          minDistance = d;
          index[0] = i;
          index[1] = j;
  ...
  public static void main(String[] args) {
    ...
    System.out.println(Room.distance(r, r2));
  }
  ...
  /*the wall space of a room*/
  public List<Position> wall() {
    HashMap<String, Integer> size = this.sizeValue();
    // left - bottom
    Position p1 = new Position(size.get("minWidth") - 1, size.get("minHeight") - 1);
    // right - top
    Position p2 = new Position(size.get("maxWidth") + 1, size.get("maxHeight") + 1);
